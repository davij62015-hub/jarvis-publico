...
async function api(url,opt={}){const r=await fetch(url,opt);let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d.erro||'Erro');return d}
async function loadServers(){try{servers=await api('/api/ng/servidores');renderServers();if(servers.length){const s=servers.find(x=>x.id===currentServer)||servers[0];selectServer(s.id)}}catch(e){toast('Não foi possível carregar os servidores.')}}
function renderServers(){document.getElementById('servers').innerHTML=servers.map(s=>`<div class="srv" title="${esc(s.nome)}" onclick="selectServer(${s.id})">${s.icone?`<img src="${esc(s.icone)}">`:esc(s.nome.slice(0,2).toUpperCase())}</div>`).join('')}
function selectServer(id){currentServer=id;currentMode='channel';currentDM=null;const s=servers.find(x=>x.id===id);if(!s)return;document.getElementById('title').textContent=s.nome;document.getElementById('subtitle').textContent=s.verificado?'Servidor oficial verificado':'Servidor';renderChannels(s.canais||[]);if(s.canais?.length)openChannel(s.canais[0].id,s.canais[0].nome)}
function renderChannels(cs){document.getElementById('channels').innerHTML=cs.map(c=>`<div class="item" onclick="openChannel(${c.id},'${esc(c.nome)}')"><span class="ico">#</span><span>${esc(c.nome)}</span></div>`).join('')}
async function openChannel(id,nome){currentMode='channel';currentChannel=id;currentDM=null;document.getElementById('title').textContent=nome;document.getElementById('subtitle').textContent='Canal';document.getElementById('input').placeholder='Mensagem em #'+nome;await loadMessages(`/api/ng/canal/${id}/mensagens`)}
async function openAI(){currentMode='ai';currentChannel=null;currentDM=null;document.getElementById('title').textContent='New GG';document.getElementById('subtitle').innerHTML='Assistente oficial <b class="verified">OFICIAL</b>';document.getElementById('input').placeholder='Pergunte qualquer coisa...';await loadMessages('/api/ng/ia/mensagens')}
async function openDM(name){name=(name||'').trim();if(!name)return toast('Digite um usuário.');closeModal('dmModal');currentMode='dm';currentDM=name;document.getElementById('title').textContent=name;document.getElementById('subtitle').textContent='Mensagem direta';document.getElementById('input').placeholder='Mensagem para '+name+'...';await loadMessages('/api/ng/dm/'+encodeURIComponent(name)+'/mensagens')}
