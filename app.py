...
function aoLoginGoogle(resposta) {
    ...
    .then(dados => {
        ...
        else { alert(dados.erro || "Nao foi possivel entrar com o Google."); }
    })
    .catch(() => alert("Falha ao conectar com o Google."));
}
...
def pagina_login(erro=""):
    if GOOGLE_CLIENT_ID:
        ...
        bloco_google = ""
    return PAGINA_LOGIN.replace("{erro}", erro).replace("{bloco_google}", bloco_google)
...
def login():
    if request.method == "POST":
        ...
        if linha and check_password_hash(linha["senha_hash"], senha):
            ...
            return redirect(url_for("carregando"))
        return pagina_login("Email ou senha incorretos.")
...
def auth_google():
    ...
    conexao.execute(
        "INSERT INTO usuarios (usuario, senha_hash, verificado, email, foto_perfil) VALUES (?, ?, 0, ?, ?)",
        (usuario_final, senha_aleatoria, email, foto),
    )
    conexao.commit()
    conexao.close()
