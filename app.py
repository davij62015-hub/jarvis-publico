...
CONTA_OFICIAL = "New GG"
EMAIL_CONTA_OFICIAL = "oficial@newgg.sistema"
...
def iniciar_bd():
    ...
    """)
    # ---------- Grupos do ZAP (servidores, com canais de texto e voz) ----------
    conexao.execute("""
    ...
    """)
    for coluna, tipo in [
        ("codigo_convite", "TEXT"), ("ativado", "INTEGER DEFAULT 0"),
        ("passo_convite", "INTEGER DEFAULT 0"), ("passo_mensagem", "INTEGER DEFAULT 0"),
        ("passo_confirmar", "INTEGER DEFAULT 0"),
    ]:
        try:
            conexao.execute(f"ALTER TABLE zap_grupos ADD COLUMN {coluna} {tipo}")
        except sqlite3.OperationalError:
            pass
    conexao.execute("""
    ...
    try:
        ...
        pass
    try:
        conexao.execute("ALTER TABLE zap_grupo_membros ADD COLUMN silenciado INTEGER DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    conexao.execute("""
    ...
    """)
    try:
        conexao.execute("ALTER TABLE zap_grupo_mensagens ADD COLUMN canal_id INTEGER")
    except sqlite3.OperationalError:
        pass
    # ---------- Canais dentro de um servidor (texto ou voz) ----------
    conexao.execute("""
        CREATE TABLE IF NOT EXISTS zap_canais (
            id INTEGER PRIMARY KEY AUTOINCREMENT, grupo_id INTEGER NOT NULL,
            nome TEXT NOT NULL, tipo TEXT NOT NULL DEFAULT 'texto', criado_em TEXT NOT NULL
        )
    """)
    # ---------- Quem esta dentro de um canal de voz agora ----------
    conexao.execute("""
        CREATE TABLE IF NOT EXISTS zap_canal_presenca (
            canal_id INTEGER NOT NULL, usuario TEXT NOT NULL, entrou_em TEXT NOT NULL,
            camera INTEGER DEFAULT 0, tela INTEGER DEFAULT 0, mudo INTEGER DEFAULT 0,
            PRIMARY KEY (canal_id, usuario)
        )
    """)
    # ---------- Sinalizacao WebRTC (malha) dos canais de voz ----------
    conexao.execute("""
        CREATE TABLE IF NOT EXISTS zap_canal_sinal (
            id INTEGER PRIMARY KEY AUTOINCREMENT, canal_id INTEGER NOT NULL,
            de_usuario TEXT NOT NULL, para_usuario TEXT NOT NULL,
            tipo TEXT NOT NULL, dados TEXT NOT NULL, criado_em TEXT NOT NULL
        )
    """)
    ...
    # ---------- Conta oficial "New GG" (tipo o bot/conta "Discord Oficial") ----------
    existe_tag_oficial = conexao.execute("SELECT 1 FROM tags WHERE nome = 'OFICIAL'").fetchone()
    if not existe_tag_oficial:
        conexao.execute("INSERT INTO tags (nome, cor, foto) VALUES ('OFICIAL', '#5865f2', NULL)")
    existe_conta_oficial = conexao.execute("SELECT 1 FROM usuarios WHERE usuario = ? COLLATE NOCASE", (CONTA_OFICIAL,)).fetchone()
    if not existe_conta_oficial:
        id_oficial = gerar_id_publico(conexao)
        senha_falsa = generate_password_hash(uuid.uuid4().hex)
        conexao.execute(
            "INSERT INTO usuarios (usuario, senha_hash, verificado, email, foto_perfil, bio, tag, id_publico) VALUES (?, ?, 1, ?, ?, ?, 'OFICIAL', ?)",
            (CONTA_OFICIAL, senha_falsa, EMAIL_CONTA_OFICIAL, "/static/logo.jpg", "Conta oficial do New GG. Avisos importantes chegam por aqui.", id_oficial),
        )
    else:
        conexao.execute("UPDATE usuarios SET verificado = 1, tag = 'OFICIAL' WHERE usuario = ? COLLATE NOCASE", (CONTA_OFICIAL,))
...
def enviar_email_generico(destino, assunto, corpo):
    """Mesma logica de fallback (Resend -> SMTP -> so log) do enviar_email_codigo,
    mas para qualquer aviso - usado no suporte por email."""
    if RESEND_API_KEY and requests is not None:
        try:
            return _enviar_email_via_resend(destino, assunto, corpo)
        except Exception as erro:
            print(f"[NEW GG] falha ao enviar email via Resend para {destino}: {erro}")
    if not (SMTP_HOST and SMTP_USUARIO and SMTP_SENHA):
        print(f"[NEW GG] (email nao configurado) para {destino} - {assunto}:\n{corpo}")
        return False
    try:
        mensagem = MIMEText(corpo)
        mensagem["Subject"] = assunto
        ...
        with smtplib.SMTP_SSL(SMTP_HOST, SMTP_PORT, context=contexto, timeout=10) as servidor:
            servidor.login(SMTP_USUARIO, SMTP_SENHA)
            servidor.sendmail(SMTP_REMETENTE, [destino], mensagem.as_string())
        return True
    except Exception as erro:
        print(f"[NEW GG] falha ao enviar email para {destino}: {erro}")
        return False


def enviar_email_generico_async(destino, assunto, corpo):
    threading.Thread(target=enviar_email_generico, args=(destino, assunto, corpo), daemon=True).start()


def notificar_suporte_por_email(usuario, texto):
    """Manda um email pro dono do site avisando que um chamado de suporte novo
    foi aberto - assim ele fica sabendo mesmo sem estar logado no site."""
    if not email_esta_configurado():
        return
    linha = buscar_usuario(usuario)
    email_usuario = linha["email"] if linha else "sem email"
    assunto = f"Novo chamado de suporte - {usuario}"
    corpo = (
        f"{usuario} ({email_usuario}) abriu um chamado de suporte no New GG:\n\n"
        f"{texto}\n\n"
        "Responda pelo painel de Suporte no site (logado como atendente/dev)."
    )
    enviar_email_generico_async(EMAIL_DONO, assunto, corpo)
...
def criar_usuario(usuario, email, foto_perfil=None, banner=None, bio=None, data_nascimento=None, id_publico=None):
    ...
    conexao.commit()
    conexao.close()
    if usuario.lower() != CONTA_OFICIAL.lower():
        try:
            enviar_mensagem_oficial(
                usuario,
                f"Boas-vindas ao New GG, {usuario}! Essa e a conta oficial - avisos importantes e novidades chegam por aqui. "
                "Aproveita pra adicionar uns amigos pelo nick ou pelo # ID deles e criar seu primeiro servidor.",
            )
        except Exception as erro:
            print(f"[NEW GG] falha ao mandar mensagem de boas-vindas para {usuario}: {erro}")
    return usuario
...
def enviar_mensagem_oficial(usuario_destino, texto):
    """Manda uma mensagem no ZAP como se fosse a conta oficial 'New GG' (tipo os
    avisos automaticos que o 'Discord OFICIAL' manda). Cria a amizade na hora,
    sem precisar de pedido, pra mensagem ja aparecer na caixa da pessoa."""
    if usuario_destino.lower() == CONTA_OFICIAL.lower():
        return
    agora = datetime.now().isoformat()
    conexao = obter_bd()
    conexao.execute("INSERT OR IGNORE INTO zap_contatos (usuario, contato, criado_em) VALUES (?, ?, ?)", (usuario_destino, CONTA_OFICIAL, agora))
    conexao.execute("INSERT OR IGNORE INTO zap_contatos (usuario, contato, criado_em) VALUES (?, ?, ?)", (CONTA_OFICIAL, usuario_destino, agora))
    conversa = id_conversa(CONTA_OFICIAL, usuario_destino)
    conexao.execute(
        "INSERT INTO zap_mensagens (conversa, remetente, destinatario, tipo, conteudo, criptografado, criado_em) VALUES (?, ?, ?, 'texto', ?, 0, ?)",
        (conversa, CONTA_OFICIAL, usuario_destino, texto, agora),
    )
    conexao.commit()
    conexao.close()
...
<div style="padding:10px 16px;font-size:12px;color:#888;border-bottom:1px solid #ffffff14;flex-shrink:0;">
  Prefere por email? Escreva pra <a href="mailto:{email_suporte}" style="color:#ccc;">{email_suporte}</a> - a gente responde por la tambem.
</div>
{painel_admin}
<div class="container">
...
.item-grupo .selo-ativado { font-size:10px; color:#3ddc6a; }
.botao-criar-servidor { width:100%; padding:14px; border-radius:10px; border:2px dashed #ffffff33; background:none; color:#fff; font-weight:bold; cursor:pointer; margin-bottom:16px; }
.modal-criar-servidor { display:none; position:fixed; inset:0; background:#000000cc; z-index:50; align-items:center; justify-content:center; padding:16px; }
.modal-criar-servidor.aberto { display:flex; }
.caixa-criar-servidor { background:#0d0d0d; border:1px solid #ffffff33; border-radius:14px; padding:22px; width:100%; max-width:360px; text-align:center; }
.caixa-criar-servidor h3 { margin-top:0; }
.avatar-escolher-servidor { width:88px; height:88px; border-radius:50%; background:#1a1a1a; margin:0 auto 14px; display:flex; align-items:center; justify-content:center; cursor:pointer; overflow:hidden; border:2px dashed #ffffff33; font-size:12px; color:#888; }
.avatar-escolher-servidor img { width:100%; height:100%; object-fit:cover; }
.caixa-criar-servidor input[type=text] { width:100%; padding:12px; border-radius:8px; border:1px solid #ffffff22; background:#000; color:#f2f2f2; margin-bottom:14px; }
.caixa-criar-servidor .linha-botoes { display:flex; gap:8px; }
.caixa-criar-servidor button { flex:1; padding:11px; border-radius:8px; border:none; font-weight:bold; cursor:pointer; }
.caixa-criar-servidor .cancelar { background:#1a1a1a; color:#fff; }
.caixa-criar-servidor .confirmar { background:#5865f2; color:#fff; }
</style></head>
<body>
<div class="topo"><a href="/app">&#8592;</a>Servidores</div>
<div class="container">
  <button class="botao-criar-servidor" onclick="abrirModalCriarServidor()">+ Criar servidor</button>
  <div id="listaGrupos"></div>
</div>

<div class="modal-criar-servidor" id="modalCriarServidor">
  <div class="caixa-criar-servidor">
    <h3>Criar servidor</h3>
    <label class="avatar-escolher-servidor" id="rotuloFotoServidor">
      Escolher foto
      <input type="file" id="fotoNovoServidor" accept="image/*" style="display:none" onchange="previaFotoServidor(this)">
    </label>
    <input type="text" id="nomeNovoServidor" placeholder="Nome do servidor" maxlength="60">
    <div class="linha-botoes">
      <button class="cancelar" onclick="fecharModalCriarServidor()">Cancelar</button>
...
<script>
function abrirModalCriarServidor() { document.getElementById("modalCriarServidor").classList.add("aberto"); }
function fecharModalCriarServidor() { document.getElementById("modalCriarServidor").classList.remove("aberto"); }
function previaFotoServidor(input) {
    const rotulo = document.getElementById("rotuloFotoServidor");
    const arquivo = input.files[0];
    if (arquivo) rotulo.innerHTML = `<img src="${URL.createObjectURL(arquivo)}">`;
}
async function criarServidor() {
    const nome = document.getElementById("nomeNovoServidor").value.trim();
    if (!nome) { alert("Digite um nome para o servidor."); return; }
    const form = new FormData();
    form.append("nome", nome);
    const arquivo = document.getElementById("fotoNovoServidor").files[0];
    if (arquivo) form.append("foto", arquivo);
    const r = await fetch("/zap/grupos/criar", { method: "POST", body: form });
    const d = await r.json();
...
async function carregarGrupos() {
    ...
    div.innerHTML = grupos.map(g => `<div class="item-grupo" onclick="window.location.href='/zap/grupo/${g.id}'"><img src="${g.foto || ''}"><div><div>${g.nome}${g.verificado ? ' &#9989;' : ''}</div>${g.ativado ? '<div class="selo-ativado">&#9989; Servidor ativado</div>' : '<div style=\\'color:#888;font-size:11px;\\'>Ativacao pendente</div>'}</div></div>`).join("") || "<div style='color:#777;'>Voce ainda nao tem servidores.</div>";
}
carregarGrupos();
</script>
</body></html>
...
:root { --bg0:#0a0a0d; --bg1:#111214; --bg2:#17181c; --bg3:#23252b; --acento:#5865f2; --online:#3ddc6a; --fraco:#9a9ca3; }
body { display:flex; height:100vh; overflow:hidden; background:var(--bg1); }
.sidebar-servidor { width:240px; background:var(--bg2); display:flex; flex-direction:column; flex-shrink:0; border-right:1px solid #ffffff0f; }
.topo-servidor { padding:14px 14px; border-bottom:1px solid #ffffff0f; display:flex; align-items:center; gap:10px; font-weight:bold; }
.topo-servidor a { color:#fff; text-decoration:none; font-size:18px; }
.botao-info-grupo { margin-left:auto; background:none; border:none; color:#fff; font-size:16px; cursor:pointer; }
.banner-ativacao { background:#5865f222; border-bottom:1px solid #5865f244; padding:10px 12px; font-size:12px; }
.banner-ativacao .passo { display:flex; align-items:center; gap:6px; margin:4px 0; color:var(--fraco); }
.banner-ativacao .passo.feito { color:var(--online); }
.banner-ativacao button { margin-top:8px; width:100%; padding:8px; border-radius:6px; border:none; background:var(--acento); color:#fff; font-weight:bold; cursor:pointer; font-size:12px; }
.banner-ativacao button:disabled { opacity:.4; cursor:default; }
.lista-canais { flex:1; overflow-y:auto; padding:8px; }
.rotulo-canais { display:flex; justify-content:space-between; align-items:center; font-size:10px; color:#6b6d75; text-transform:uppercase; padding:10px 6px 4px; }
.rotulo-canais span.add { cursor:pointer; font-size:14px; }
.item-canal { display:flex; align-items:center; gap:6px; padding:7px 8px; border-radius:6px; cursor:pointer; color:var(--fraco); font-size:14px; }
.item-canal:hover, .item-canal.ativo { background:var(--bg3); color:#fff; }
.item-canal .contagem-voz { margin-left:auto; font-size:11px; color:#6b6d75; }
.rodape-servidor { padding:10px; border-top:1px solid #ffffff0f; font-size:12px; color:var(--fraco); }
.rodape-servidor button { width:100%; padding:8px; border-radius:6px; border:1px solid #ff5c5c55; background:none; color:#ff8a8a; cursor:pointer; font-size:12px; }

.area-principal { flex:1; display:flex; flex-direction:column; min-width:0; }
.topo-canal { padding:14px 18px; border-bottom:1px solid #ffffff0f; font-weight:bold; display:flex; align-items:center; gap:8px; }
.msgs-zap { flex:1; overflow-y:auto; padding:18px; display:flex; flex-direction:column; gap:10px; }
...
.area-input-zap { padding:12px 16px calc(12px + env(safe-area-inset-bottom)); border-top:1px solid #ffffff0f; display:flex; gap:8px; }
.area-input-zap input[type=text] { flex:1; padding:12px 14px; border-radius:20px; border:none; background:var(--bg2); color:#f2f2f2; }
.area-input-zap button { background:var(--acento); color:#fff; border:none; border-radius:50%; width:40px; height:40px; cursor:pointer; }

.sala-voz { flex:1; display:flex; flex-direction:column; }
.grade-voz { flex:1; display:flex; flex-wrap:wrap; align-items:center; justify-content:center; gap:16px; padding:20px; }
.tile-voz { width:180px; height:135px; background:var(--bg2); border-radius:12px; border:2px solid transparent; display:flex; flex-direction:column; align-items:center; justify-content:center; position:relative; overflow:hidden; }
.tile-voz.falando { border-color:var(--online); }
.tile-voz video { width:100%; height:100%; object-fit:cover; position:absolute; inset:0; display:none; }
.tile-voz video.ativo { display:block; }
.tile-voz img.avatar-voz { width:56px; height:56px; border-radius:50%; object-fit:cover; z-index:1; }
.tile-voz .nome-voz { z-index:1; font-size:12px; margin-top:6px; background:#00000088; padding:2px 8px; border-radius:10px; position:absolute; bottom:6px; left:6px; display:flex; align-items:center; gap:4px; }
.tile-voz .icone-mudo { color:#ff6b6b; font-size:11px; }
.tile-voz .btn-silenciar-outro { position:absolute; top:6px; right:6px; background:#000000aa; border:none; color:#fff; font-size:10px; padding:3px 6px; border-radius:6px; cursor:pointer; z-index:2; }
.barra-controle-voz { padding:16px; display:flex; justify-content:center; gap:14px; border-top:1px solid #ffffff0f; flex-wrap:wrap; }
.barra-controle-voz button { width:48px; height:48px; border-radius:50%; border:none; font-size:18px; cursor:pointer; background:var(--bg3); color:#fff; }
.barra-controle-voz button.ativo-func { background:var(--acento); }
.barra-controle-voz button.sair-voz { background:#ed4245; }
.barra-controle-voz button:disabled { opacity:.4; }
...
.folha-grupo-conteudo { width:100%; max-width:480px; max-height:82vh; background:var(--bg2); border-radius:16px 16px 0 0; border:1px solid #ffffff22; border-bottom:none; display:flex; flex-direction:column; overflow-y:auto; padding:16px; }
...
.linha-convite { display:flex; gap:6px; margin-bottom:16px; }
.linha-convite input { flex:1; padding:9px; border-radius:8px; border:1px solid #ffffff22; background:#000; color:#f2f2f2; font-size:12px; }
.linha-convite button { padding:9px 12px; border-radius:8px; border:none; background:var(--acento); color:#fff; font-weight:bold; cursor:pointer; font-size:12px; }
...
.linha-membro-grupo .tag-admin-membro { font-size:10px; color:var(--online); margin-left:6px; }
.linha-membro-grupo .tag-silenciado-membro { font-size:10px; color:#ff6b6b; margin-left:6px; }
.linha-membro-grupo button { background:var(--bg3); border:1px solid #ffffff22; color:#f2f2f2; font-size:11px; padding:5px 8px; border-radius:6px; cursor:pointer; margin-left:4px; }
...
@media (max-width:760px) {
  .sidebar-servidor { position:fixed; z-index:20; height:100vh; transition:margin-left .2s; }
  .sidebar-servidor.recolhida { margin-left:-240px; }
  .tile-voz { width:140px; height:105px; }
}
</style></head>
<body>
<div class="sidebar-servidor" id="sidebarServidor">
  <div class="topo-servidor">
    <span class="botao-voltar-lista" onclick="document.getElementById('sidebarServidor').classList.toggle('recolhida')" style="cursor:pointer;">&#9776;</span>
    <a href="/zap/grupos">&#8592;</a><b>{nome_grupo}</b>{selo_dev_grupo}
    <button class="botao-info-grupo" onclick="abrirInfoGrupo()" title="Configuracoes do servidor">&#9881;</button>
  </div>
  <div class="banner-ativacao" id="bannerAtivacao" style="display:none;">
    <b>Ativar servidor</b>
    <div class="passo" id="passoConvidar">&#9675; Convidar alguem pro servidor</div>
    <div class="passo" id="passoMensagem">&#9675; Mandar uma mensagem</div>
    <button id="botaoConfirmarAtivacao" onclick="confirmarAtivacao()" disabled>Concluir ativacao (ok)</button>
  </div>
  <div class="lista-canais" id="listaCanais"></div>
  <div class="rodape-servidor"><button onclick="sairDoGrupo()">Sair do servidor</button></div>
</div>

<div class="area-principal" id="areaPrincipal">
  <div style="flex:1;display:flex;align-items:center;justify-content:center;color:#666;">Escolha um canal na lista.</div>
</div>

<div class="folha-grupo" id="folhaGrupo" onclick="fecharInfoGrupo()">
  <div class="folha-grupo-conteudo" onclick="event.stopPropagation()">
    <div class="folha-grupo-topo"><b>Informacoes do servidor</b><span onclick="fecharInfoGrupo()">&times;</span></div>
    ...
    <div class="info-grupo-nome">
      <input type="text" id="nomeGrupoInput" placeholder="Nome do servidor">
      <button id="botaoSalvarNomeGrupo" style="display:none;" onclick="salvarInfoGrupo()">Salvar</button>
    </div>
    <div class="linha-convite">
      <input type="text" id="linkConviteGrupo" readonly>
      <button onclick="copiarConvite()">Copiar convite</button>
    </div>
    ...
    <button class="botao-sair-grupo" onclick="sairDoGrupo()">Sair do servidor</button>
  </div>
</div>
<script>
...
let canalAtual = null, tipoCanalAtual = null;
function escaparHtml(t) { const d = document.createElement("div"); d.textContent = t; return d.innerHTML; }

// ---------------- Canais ----------------
async function carregarCanais() {
    const r = await fetch("/zap/grupo/" + grupoId + "/canais");
    ...
    let html = '<div class="rotulo-canais">Canais de texto' + (souAdminDoGrupo ? '<span class="add" onclick="criarCanal(\\'texto\\')">+</span>' : '') + '</div>';
    html += texto.map(c => `<div class="item-canal ${canalAtual === c.id ? 'ativo' : ''}" onclick="abrirCanalTexto(${c.id}, '${escaparHtml(c.nome)}')">#${escaparHtml(c.nome)}</div>`).join("");
    html += '<div class="rotulo-canais">Canais de voz' + (souAdminDoGrupo ? '<span class="add" onclick="criarCanal(\\'voz\\')">+</span>' : '') + '</div>';
    html += voz.map(c => `<div class="item-canal ${canalAtual === c.id ? 'ativo' : ''}" onclick="abrirCanalVoz(${c.id}, '${escaparHtml(c.nome)}')">&#128266; ${escaparHtml(c.nome)}<span class="contagem-voz">${c.presentes > 0 ? c.presentes : ''}</span></div>`).join("");
    div.innerHTML = html;
}
async function criarCanal(tipo) {
    const nome = prompt(tipo === "voz" ? "Nome do canal de voz:" : "Nome do canal de texto:");
    if (!nome) return;
    const r = await fetch("/zap/grupo/" + grupoId + "/canal/criar", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({nome, tipo}) });
    const d = await r.json();
    if (!d.ok) { alert(d.erro || "Nao foi possivel criar o canal."); return; }
    carregarCanais();
}

function abrirCanalTexto(id, nome) {
    if (canalAtual !== null && tipoCanalAtual === "voz") sairDoCanalVoz();
    canalAtual = id; tipoCanalAtual = "texto";
    carregarCanais();
    document.getElementById("areaPrincipal").innerHTML = `
      <div class="topo-canal">#${escaparHtml(nome)}</div>
      <div class="msgs-zap" id="msgsGrupo"></div>
      <div class="area-input-zap">
        <input type="text" id="campoGrupo" placeholder="Mensagem em #${escaparHtml(nome)}" onkeydown="if(event.key==='Enter')enviarMsgGrupo()">
        <button onclick="enviarMsgGrupo()">&#10148;</button>
      </div>`;
    carregarMsgsGrupo();
    if (window.pollMsgsGrupo) clearInterval(window.pollMsgsGrupo);
    window.pollMsgsGrupo = setInterval(carregarMsgsGrupo, 3500);
    if (window.innerWidth <= 760) document.getElementById("sidebarServidor").classList.add("recolhida");
}
async function carregarMsgsGrupo() {
    if (tipoCanalAtual !== "texto") return;
    const r = await fetch("/zap/grupo/" + grupoId + "/canal/" + canalAtual + "/mensagens");
    const d = await r.json();
    const caixa = document.getElementById("msgsGrupo");
    if (!caixa) return;
    caixa.innerHTML = d.mensagens.map(m => `<div class="bolha ${m.minha ? 'minha' : 'dele'}">${!m.minha ? '<div class=\\"remetente\\">' + escaparHtml(m.remetente) + '</div>' : ''}${escaparHtml(m.conteudo)}</div>`).join("");
    caixa.scrollTop = caixa.scrollHeight;
    carregarStatusAtivacao();
}
async function enviarMsgGrupo() {
    ...
    if (!texto) return;
    campo.value = "";
    await fetch("/zap/grupo/" + grupoId + "/canal/" + canalAtual + "/enviar", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({texto}) });
    carregarMsgsGrupo();
}

// ---------------- Ativacao do servidor ----------------
async function carregarStatusAtivacao() {
    const r = await fetch("/zap/grupo/" + grupoId + "/status_ativacao");
    const d = await r.json();
    if (!d.ok) return;
    document.getElementById("linkConviteGrupo").value = location.origin + "/zap/grupo/entrar/" + d.codigo_convite;
    const banner = document.getElementById("bannerAtivacao");
    if (d.ativado) { banner.style.display = "none"; return; }
    banner.style.display = "block";
    document.getElementById("passoConvidar").className = "passo" + (d.passo_convite ? " feito" : "");
    document.getElementById("passoConvidar").innerHTML = (d.passo_convite ? "&#9989;" : "&#9675;") + " Convidar alguem pro servidor";
    document.getElementById("passoMensagem").className = "passo" + (d.passo_mensagem ? " feito" : "");
    document.getElementById("passoMensagem").innerHTML = (d.passo_mensagem ? "&#9989;" : "&#9675;") + " Mandar uma mensagem";
    document.getElementById("botaoConfirmarAtivacao").disabled = !(d.passo_convite && d.passo_mensagem);
}
async function confirmarAtivacao() {
    const r = await fetch("/zap/grupo/" + grupoId + "/confirmar_ativacao", { method: "POST" });
    const d = await r.json();
    if (!d.ok) { alert(d.erro || "Complete os passos primeiro."); return; }
    carregarStatusAtivacao();
}
function copiarConvite() {
    const campo = document.getElementById("linkConviteGrupo");
    campo.select();
    navigator.clipboard.writeText(campo.value);
    alert("Link de convite copiado!");
}

// ---------------- Info / membros do servidor ----------------
async function abrirInfoGrupo() {
    ...
    lista.innerHTML = d.membros.map(m => {
        let botoes = "";
        if (souAdminDoGrupo && m.usuario !== meuUsuario) {
            botoes += `<button onclick="promoverMembroGrupo('${m.usuario}')">${m.admin ? "Remover admin" : "Tornar admin"}</button>`;
            botoes += `<button onclick="silenciarMembroGrupo('${m.usuario}')">${m.silenciado ? "Dessilenciar" : "Silenciar"}</button>`;
            botoes += `<button onclick="removerMembroGrupo('${m.usuario}')">Remover</button>`;
        }
        return `<div class="linha-membro-grupo"><img src="${m.foto || ''}"><span class="nome-membro">${escaparHtml(m.usuario)}${m.admin ? '<span class="tag-admin-membro">ADMIN</span>' : ''}${m.silenciado ? '<span class="tag-silenciado-membro">SILENCIADO</span>' : ''}</span>${botoes}</div>`;
    }).join("");
    carregarCanais();
}
...
async function salvarInfoGrupo() {
    ...
    document.querySelector(".topo-servidor b").textContent = d.nome;
    abrirInfoGrupo();
}
...
async function removerMembroGrupo(usuario) {
    if (!confirm("Remover " + usuario + " do servidor?")) return;
...
async function silenciarMembroGrupo(usuario) {
    const r = await fetch("/zap/grupo/" + grupoId + "/membro/silenciar", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({usuario}) });
    const d = await r.json();
    if (!d.ok) { alert(d.erro || "Nao foi possivel silenciar."); return; }
    abrirInfoGrupo();
}
async function sairDoGrupo() {
    ...
    const d = await r.json();
    if (!d.ok) { alert(d.erro || "Nao foi possivel sair do servidor."); return; }
    window.location.href = "/zap/grupos";
}

// ================= CANAL DE VOZ (WebRTC em malha) =================
const CONFIG_ICE = { iceServers: """ + ICE_SERVERS_JSON + """ };
let peers = {};      // usuario -> RTCPeerConnection
let streamLocalVoz = null;
let telaLocalTrack = null;
let indiceSinalRecebido = 0;
let pollSinaisVoz = null, pollParticipantesVoz = null;
let meuMudo = false, minhaCamera = false, minhaTela = false;

function abrirCanalVoz(id, nome) {
    if (canalAtual !== null && tipoCanalAtual === "voz" && canalAtual !== id) sairDoCanalVoz();
    if (canalAtual === id && tipoCanalAtual === "voz") return;
    canalAtual = id; tipoCanalAtual = "voz";
    if (window.pollMsgsGrupo) { clearInterval(window.pollMsgsGrupo); window.pollMsgsGrupo = null; }
    carregarCanais();
    document.getElementById("areaPrincipal").innerHTML = `
      <div class="topo-canal">&#128266; ${escaparHtml(nome)}</div>
      <div class="sala-voz">
        <div class="grade-voz" id="gradeVoz"></div>
        <div class="barra-controle-voz">
          <button id="botaoMudoVoz" onclick="alternarMudoVoz()" title="Mutar/desmutar">&#127908;</button>
          <button id="botaoCameraVoz" onclick="alternarCameraVoz()" title="Camera">&#128249;</button>
          <button id="botaoTelaVoz" onclick="alternarTelaVoz()" title="Compartilhar tela">&#128421;</button>
          <button class="sair-voz" onclick="sairDoCanalVoz()" title="Sair do canal">&#128222;</button>
        </div>
      </div>`;
    entrarNoCanalVoz();
    if (window.innerWidth <= 760) document.getElementById("sidebarServidor").classList.add("recolhida");
...
async function entrarNoCanalVoz() {
    try {
        streamLocalVoz = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
    } catch (e) {
        alert("Nao foi possivel acessar o microfone.");
        return;
    }
    const r = await fetch("/zap/canal/" + canalAtual + "/entrar", { method: "POST" });
    const d = await r.json();
    if (d.forcado_mudo) {
        meuMudo = true;
        streamLocalVoz.getAudioTracks().forEach(t => t.enabled = false);
    }
    atualizarBotaoMudo();
    indiceSinalRecebido = 0;
    renderizarGradeVoz([]);
    pollParticipantesVoz = setInterval(atualizarParticipantesVoz, 2000);
    pollSinaisVoz = setInterval(processarSinaisVoz, 1200);
    atualizarParticipantesVoz();
}

function tileVozHtml(p) {
    return `<div class="tile-voz" id="tileVoz_${p.usuario}">
      <video id="video_${p.usuario}" autoplay playsinline ${p.usuario === meuUsuario ? "muted" : ""}></video>
      <img class="avatar-voz" src="${p.avatar}">
      <div class="nome-voz">${p.mudo ? '<span class="icone-mudo">&#128263;</span>' : ''}${escaparHtml(p.usuario)}</div>
      ${(souAdminDoGrupo && p.usuario !== meuUsuario) ? `<button class="btn-silenciar-outro" onclick="silenciarMembroGrupo('${p.usuario}')">${p.silenciado_pelo_admin ? 'Dessilenciar' : 'Silenciar'}</button>` : ''}
    </div>`;
}
function renderizarGradeVoz(participantes) {
    document.getElementById("gradeVoz").innerHTML = participantes.map(tileVozHtml).join("");
    participantes.forEach(p => {
        if (p.usuario === meuUsuario && streamLocalVoz) {
            const v = document.getElementById("video_" + p.usuario);
            if (v) { v.srcObject = streamLocalVoz; v.classList.toggle("ativo", minhaCamera || minhaTela); }
        } else if (peers[p.usuario] && peers[p.usuario]._streamRemoto) {
            const v = document.getElementById("video_" + p.usuario);
            if (v) { v.srcObject = peers[p.usuario]._streamRemoto; v.classList.toggle("ativo", p.camera || p.tela); }
        }
    });
}
...
async function atualizarParticipantesVoz() {
    if (tipoCanalAtual !== "voz") return;
    const r = await fetch("/zap/canal/" + canalAtual + "/participantes");
    const participantes = await r.json();
    ultimosParticipantes = participantes;
    renderizarGradeVoz(participantes);
    const usuariosAtuais = new Set(participantes.map(p => p.usuario));
    // conecta com quem chegou e ainda nao temos conexao (soh o de "nome menor" oferta, evita oferta dupla)
    for (const p of participantes) {
        if (p.usuario === meuUsuario || peers[p.usuario]) continue;
        if (meuUsuario < p.usuario) await iniciarConexaoVoz(p.usuario);
    }
    // fecha conexoes de quem saiu
    for (const usuario of Object.keys(peers)) {
        if (!usuariosAtuais.has(usuario)) fecharConexaoVoz(usuario);
    }
    // aplica mudo forcado pelo admin em mim
    const eu = participantes.find(p => p.usuario === meuUsuario);
    if (eu && eu.silenciado_pelo_admin && !meuMudo) {
        meuMudo = true;
        if (streamLocalVoz) streamLocalVoz.getAudioTracks().forEach(t => t.enabled = false);
        atualizarBotaoMudo();
    }
}

function novaConexaoVoz(usuario) {
    const pc = new RTCPeerConnection(CONFIG_ICE);
    pc._streamRemoto = new MediaStream();
    if (streamLocalVoz) streamLocalVoz.getTracks().forEach(t => pc.addTrack(t, streamLocalVoz));
    pc.ontrack = (ev) => {
        pc._streamRemoto.addTrack(ev.track);
        const v = document.getElementById("video_" + usuario);
        if (v) { v.srcObject = pc._streamRemoto; if (ev.track.kind === "video") v.classList.add("ativo"); }
    };
    pc.onicecandidate = (ev) => {
        if (ev.candidate) enviarSinalVoz(usuario, "candidato", ev.candidate);
    };
    peers[usuario] = pc;
    return pc;
}
async function iniciarConexaoVoz(usuario) {
    const pc = novaConexaoVoz(usuario);
    const oferta = await pc.createOffer();
    await pc.setLocalDescription(oferta);
    enviarSinalVoz(usuario, "oferta", oferta);
}
async function enviarSinalVoz(para, tipo, dados) {
    await fetch("/zap/canal/" + canalAtual + "/sinal", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({para, tipo, dados}) });
}
async function processarSinaisVoz() {
    if (tipoCanalAtual !== "voz") return;
    const r = await fetch("/zap/canal/" + canalAtual + "/sinais?desde=" + indiceSinalRecebido);
    const d = await r.json();
    for (const s of d.sinais) {
        indiceSinalRecebido = Math.max(indiceSinalRecebido, s.id);
        let pc = peers[s.de];
        if (s.tipo === "oferta") {
            if (!pc) pc = novaConexaoVoz(s.de);
            await pc.setRemoteDescription(s.dados);
            const resposta = await pc.createAnswer();
            await pc.setLocalDescription(resposta);
            enviarSinalVoz(s.de, "resposta", resposta);
        } else if (s.tipo === "resposta" && pc) {
            if (!pc.currentRemoteDescription) await pc.setRemoteDescription(s.dados);
        } else if (s.tipo === "candidato" && pc) {
            try { await pc.addIceCandidate(s.dados); } catch (e) {}
        }
    }
}
function fecharConexaoVoz(usuario) {
    if (peers[usuario]) { peers[usuario].close(); delete peers[usuario]; }
}
function atualizarBotaoMudo() {
    document.getElementById("botaoMudoVoz").classList.toggle("ativo-func", !meuMudo);
    document.getElementById("botaoMudoVoz").innerHTML = meuMudo ? "&#128263;" : "&#127908;";
}
async function alternarMudoVoz() {
    if (!streamLocalVoz) return;
    const querDesmutar = meuMudo;
    const r = await fetch("/zap/canal/" + canalAtual + "/estado", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({mudo: !querDesmutar}) });
    const d = await r.json();
    if (!d.ok) { alert(d.erro || "Nao foi possivel."); return; }
    meuMudo = !querDesmutar;
    streamLocalVoz.getAudioTracks().forEach(t => t.enabled = !meuMudo);
    atualizarBotaoMudo();
}
async function alternarCameraVoz() {
    if (!streamLocalVoz) return;
    minhaCamera = !minhaCamera;
    if (minhaCamera) {
        try {
            const streamCam = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" } });
            const trackCam = streamCam.getVideoTracks()[0];
            streamLocalVoz.addTrack(trackCam);
            Object.values(peers).forEach(pc => pc.addTrack(trackCam, streamLocalVoz));
        } catch (e) { minhaCamera = false; alert("Nao foi possivel acessar a camera."); return; }
    } else {
        streamLocalVoz.getVideoTracks().forEach(t => { t.stop(); streamLocalVoz.removeTrack(t); });
    }
    document.getElementById("botaoCameraVoz").classList.toggle("ativo-func", minhaCamera);
    await fetch("/zap/canal/" + canalAtual + "/estado", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({camera: minhaCamera}) });
    atualizarParticipantesVoz();
}
async function alternarTelaVoz() {
    if (!minhaTela) {
        try {
            ...
            telaLocalTrack = streamTela.getVideoTracks()[0];
            telaLocalTrack.onended = () => alternarTelaVoz();
            for (const pc of Object.values(peers)) {
                const sender = pc.getSenders().find(s => s.track && s.track.kind === "video");
                if (sender) await sender.replaceTrack(telaLocalTrack); else pc.addTrack(telaLocalTrack, streamTela);
            }
            const v = document.getElementById("video_" + meuUsuario);
            if (v) { v.srcObject = streamTela; v.classList.add("ativo"); }
            minhaTela = true;
        } catch (e) { return; }
    } else {
        minhaTela = false;
        if (telaLocalTrack) { telaLocalTrack.stop(); telaLocalTrack = null; }
        for (const pc of Object.values(peers)) {
            const sender = pc.getSenders().find(s => s.track && s.track.kind === "video");
            if (sender) { if (streamLocalVoz.getVideoTracks()[0]) sender.replaceTrack(streamLocalVoz.getVideoTracks()[0]); else pc.removeTrack(sender); }
        }
    }
    document.getElementById("botaoTelaVoz").classList.toggle("ativo-func", minhaTela);
    await fetch("/zap/canal/" + canalAtual + "/estado", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({tela: minhaTela}) });
    atualizarParticipantesVoz();
}
async function sairDoCanalVoz() {
    if (tipoCanalAtual !== "voz" || canalAtual === null) return;
    await fetch("/zap/canal/" + canalAtual + "/sair", { method: "POST" });
    Object.keys(peers).forEach(fecharConexaoVoz);
    if (streamLocalVoz) { streamLocalVoz.getTracks().forEach(t => t.stop()); streamLocalVoz = null; }
    if (telaLocalTrack) { telaLocalTrack.stop(); telaLocalTrack = null; }
    if (pollSinaisVoz) { clearInterval(pollSinaisVoz); pollSinaisVoz = null; }
    if (pollParticipantesVoz) { clearInterval(pollParticipantesVoz); pollParticipantesVoz = null; }
    meuMudo = false; minhaCamera = false; minhaTela = false;
    tipoCanalAtual = null; canalAtual = null;
    document.getElementById("areaPrincipal").innerHTML = '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#666;">Escolha um canal na lista.</div>';
    carregarCanais();
}
window.addEventListener("beforeunload", () => { if (tipoCanalAtual === "voz" && canalAtual) navigator.sendBeacon("/zap/canal/" + canalAtual + "/sair"); });

carregarCanais();
carregarStatusAtivacao();
setInterval(carregarStatusAtivacao, 6000);
setInterval(carregarCanais, 6000);
</script>
</body></html>
"""
...
def rede():
    ...
    if eh_dev:
          ...
          <div class="painel-admin-corpo">
            ...
            <div class="painel-admin-secao" id="secaoAdmin-icones">
              ...
              <div class="lista-icones">
                ...
                <div class="item-icone"><img id="previaFotoOficial" src="{foto_oficial}"><input type="file" accept="image/*" id="arqFotoOficial" style="display:none" onchange="enviarConfig('foto_oficial','arqFotoOficial','resultadoIcones',this)"><button class="acao" style="padding:4px 8px;font-size:10px;" onclick="document.getElementById('arqFotoOficial').click()">Conta oficial New GG</button></div>
              </div>
        ...
        painel_admin_html = (
            ...
            .replace("{foto_oficial}", (buscar_usuario(CONTA_OFICIAL)["foto_perfil"] if buscar_usuario(CONTA_OFICIAL) else "/static/logo.jpg"))
        )
...
def admin_config():
    ...
    if chave == "foto_oficial":
        conexao = obter_bd()
        conexao.execute("UPDATE usuarios SET foto_perfil = ? WHERE usuario = ? COLLATE NOCASE", (url, CONTA_OFICIAL))
        conexao.commit()
        conexao.close()
    else:
...
def suporte():
    ...
    pagina = pagina.replace("{email_suporte}", EMAIL_DONO)
    return pagina
...
def suporte_abrir_chamado():
    ...
    conexao.close()
    notificar_suporte_por_email(usuario, "(chamado aberto sem mensagem inicial)")
...
def suporte_enviar():
    ...
    conexao.close()
    if ticket_novo:
        notificar_suporte_por_email(usuario, texto)
...
def zap_grupos_lista():
    ...
    conexao.close()
    return jsonify([
        {"id": l["id"], "nome": l["nome"], "foto": l["foto"], "verificado": bool(l["verificado"]), "ativado": bool(l["ativado"])}
        for l in linhas
    ])
...
def zap_grupos_criar():
    """Cria um servidor novo: nome + foto (a tela de 'escolher nome e foto').
    Ja nasce com canal de texto #geral e canal de voz 'Geral', e so fica com o
    selo de ativado depois que os 3 passos (convidar, mandar mensagem, confirmar)
    forem cumpridos."""
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    ...
    if not nome:
        return jsonify({"ok": False, "erro": "Digite um nome para o servidor."})
    foto = salvar_imagem(request.files.get("foto"))
    conexao = obter_bd()
    codigo = _gerar_codigo_convite()
    cursor = conexao.execute(
        "INSERT INTO zap_grupos (nome, foto, criado_por, verificado, criado_em, codigo_convite, ativado) VALUES (?, ?, ?, 0, ?, ?, 0)",
        (nome, foto, usuario, datetime.now().isoformat(), codigo),
    )
    grupo_id = cursor.lastrowid
    conexao.execute("INSERT OR IGNORE INTO zap_grupo_membros (grupo_id, usuario, admin) VALUES (?, ?, 1)", (grupo_id, usuario))
    _criar_canais_padrao(conexao, grupo_id)
    conexao.commit()
    conexao.close()
...
def _gerar_codigo_convite():
    ...
    conexao = obter_bd()
    while True:
        codigo = "".join(random.choice(alfabeto) for _ in range(8))
        if not conexao.execute("SELECT 1 FROM zap_grupos WHERE codigo_convite = ?", (codigo,)).fetchone():
            conexao.close()
            return codigo


def _criar_canais_padrao(conexao, grupo_id):
    agora = datetime.now().isoformat()
    conexao.execute("INSERT INTO zap_canais (grupo_id, nome, tipo, criado_em) VALUES (?, 'geral', 'texto', ?)", (grupo_id, agora))
    conexao.execute("INSERT INTO zap_canais (grupo_id, nome, tipo, criado_em) VALUES (?, 'Geral', 'voz', ?)", (grupo_id, agora))


def _canal_do_grupo(canal_id):
    conexao = obter_bd()
    linha = conexao.execute("SELECT * FROM zap_canais WHERE id = ?", (canal_id,)).fetchone()
    conexao.close()
    return linha


def _checar_ativacao_grupo(conexao, grupo_id):
    grupo = conexao.execute("SELECT passo_convite, passo_mensagem, passo_confirmar, ativado FROM zap_grupos WHERE id = ?", (grupo_id,)).fetchone()
    if grupo and grupo["passo_convite"] and grupo["passo_mensagem"] and grupo["passo_confirmar"] and not grupo["ativado"]:
        conexao.execute("UPDATE zap_grupos SET ativado = 1 WHERE id = ?", (grupo_id,))
...
def zap_grupo_info(grupo_id):
    ...
    membros = conexao.execute(
        """SELECT m.usuario, m.admin, m.silenciado, u.foto_perfil FROM zap_grupo_membros m
    ...
    return jsonify({
        ...
        "ativado": bool(grupo["ativado"]),
        "membros": [{"usuario": m["usuario"], "admin": bool(m["admin"]), "silenciado": bool(m["silenciado"]), "foto": m["foto_perfil"]} for m in membros],
    })
...
def zap_grupo_membro_adicionar(grupo_id):
    ...
    conexao.execute("UPDATE zap_grupos SET passo_convite = 1 WHERE id = ?", (grupo_id,))
    _checar_ativacao_grupo(conexao, grupo_id)
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})


@app.route("/zap/grupo/entrar/<codigo>")
def zap_grupo_entrar_por_convite(codigo):
    """Link de convite do servidor, tipo 'discord.gg/xxxx'."""
    if not session.get("usuario"):
        return redirect(url_for("login"))
    usuario = session["usuario"]
    conexao = obter_bd()
    grupo = conexao.execute("SELECT * FROM zap_grupos WHERE codigo_convite = ?", (codigo,)).fetchone()
    if not grupo:
        conexao.close()
        return "Convite invalido ou expirado.", 404
    conexao.execute("INSERT OR IGNORE INTO zap_grupo_membros (grupo_id, usuario, admin) VALUES (?, ?, 0)", (grupo["id"], usuario))
    conexao.execute("UPDATE zap_grupos SET passo_convite = 1 WHERE id = ?", (grupo["id"],))
    _checar_ativacao_grupo(conexao, grupo["id"])
    conexao.commit()
    conexao.close()
    return redirect(f"/zap/grupo/{grupo['id']}")
...
@app.route("/zap/grupo/<int:grupo_id>/membro/silenciar", methods=["POST"])
def zap_grupo_membro_silenciar(grupo_id):
    """Admin silencia (ou desfaz o silencio de) um membro. Enquanto silenciado,
    a pessoa fica sem poder ligar o microfone nos canais de voz deste servidor."""
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    if not _admin_do_grupo(grupo_id, usuario):
        return jsonify({"ok": False, "erro": "So um admin do grupo pode silenciar membros."}), 403
    dados = request.get_json() or {}
    alvo = (dados.get("usuario") or "").strip()
    if alvo.lower() == usuario.lower():
        return jsonify({"ok": False, "erro": "Voce nao pode se silenciar."})
    conexao = obter_bd()
    linha = conexao.execute("SELECT silenciado FROM zap_grupo_membros WHERE grupo_id = ? AND usuario = ? COLLATE NOCASE", (grupo_id, alvo)).fetchone()
    if not linha:
        conexao.close()
        return jsonify({"ok": False, "erro": "Essa pessoa nao esta no grupo."})
    novo_estado = 0 if linha["silenciado"] else 1
    conexao.execute("UPDATE zap_grupo_membros SET silenciado = ? WHERE grupo_id = ? AND usuario = ? COLLATE NOCASE", (novo_estado, grupo_id, alvo))
    if novo_estado:
        # forca o mudo em qualquer canal de voz deste servidor em que a pessoa esteja agora
        canais_do_grupo = [c["id"] for c in conexao.execute("SELECT id FROM zap_canais WHERE grupo_id = ? AND tipo = 'voz'", (grupo_id,)).fetchall()]
        for canal_id in canais_do_grupo:
            conexao.execute("UPDATE zap_canal_presenca SET mudo = 1 WHERE canal_id = ? AND usuario = ? COLLATE NOCASE", (canal_id, alvo))
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True, "silenciado": bool(novo_estado)})


@app.route("/zap/grupo/<int:grupo_id>/status_ativacao")
def zap_grupo_status_ativacao(grupo_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    if not _membro_do_grupo(grupo_id, session["usuario"]):
        return jsonify({"ok": False}), 403
    conexao = obter_bd()
    grupo = conexao.execute("SELECT passo_convite, passo_mensagem, passo_confirmar, ativado, codigo_convite FROM zap_grupos WHERE id = ?", (grupo_id,)).fetchone()
    conexao.close()
    if not grupo:
        return jsonify({"ok": False}), 404
    return jsonify({
        "ok": True, "ativado": bool(grupo["ativado"]), "codigo_convite": grupo["codigo_convite"],
        "passo_convite": bool(grupo["passo_convite"]), "passo_mensagem": bool(grupo["passo_mensagem"]),
        "passo_confirmar": bool(grupo["passo_confirmar"]),
    })


@app.route("/zap/grupo/<int:grupo_id>/confirmar_ativacao", methods=["POST"])
def zap_grupo_confirmar_ativacao(grupo_id):
    """O 'ok' final do assistente de ativacao do servidor - so libera depois que
    os outros dois passos (convidar e mandar mensagem) ja foram cumpridos."""
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    if not _membro_do_grupo(grupo_id, session["usuario"]):
        return jsonify({"ok": False}), 403
    conexao = obter_bd()
    grupo = conexao.execute("SELECT passo_convite, passo_mensagem FROM zap_grupos WHERE id = ?", (grupo_id,)).fetchone()
    if not grupo or not (grupo["passo_convite"] and grupo["passo_mensagem"]):
        conexao.close()
        return jsonify({"ok": False, "erro": "Complete os passos de convidar e mandar uma mensagem primeiro."})
    conexao.execute("UPDATE zap_grupos SET passo_confirmar = 1, ativado = 1 WHERE id = ?", (grupo_id,))
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})
...
@app.route("/zap/grupo/<int:grupo_id>/canais")
def zap_grupo_canais(grupo_id):
    if not session.get("usuario"):
        return jsonify([]), 401
    if not _membro_do_grupo(grupo_id, session["usuario"]):
        return jsonify([]), 403
    conexao = obter_bd()
    canais = conexao.execute("SELECT * FROM zap_canais WHERE grupo_id = ? ORDER BY tipo DESC, id ASC", (grupo_id,)).fetchall()
    conexao.close()
    resultado = []
    for c in canais:
        qtd_presentes = 0
        if c["tipo"] == "voz":
            conexao2 = obter_bd()
            qtd_presentes = conexao2.execute("SELECT COUNT(*) AS n FROM zap_canal_presenca WHERE canal_id = ?", (c["id"],)).fetchone()["n"]
            conexao2.close()
        resultado.append({"id": c["id"], "nome": c["nome"], "tipo": c["tipo"], "presentes": qtd_presentes})
    return jsonify(resultado)


@app.route("/zap/grupo/<int:grupo_id>/canal/criar", methods=["POST"])
def zap_grupo_canal_criar(grupo_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    if not _admin_do_grupo(grupo_id, session["usuario"]):
        return jsonify({"ok": False, "erro": "So um admin do servidor pode criar canais."}), 403
    dados = request.get_json() or {}
    nome = (dados.get("nome") or "").strip()
    tipo = dados.get("tipo") if dados.get("tipo") in ("texto", "voz") else "texto"
    if not nome:
        return jsonify({"ok": False, "erro": "Digite um nome para o canal."})
    conexao = obter_bd()
    cursor = conexao.execute("INSERT INTO zap_canais (grupo_id, nome, tipo, criado_em) VALUES (?, ?, ?, ?)", (grupo_id, nome, tipo, datetime.now().isoformat()))
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True, "canal_id": cursor.lastrowid})


@app.route("/zap/grupo/<int:grupo_id>/canal/<int:canal_id>/excluir", methods=["POST"])
def zap_grupo_canal_excluir(grupo_id, canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    if not _admin_do_grupo(grupo_id, session["usuario"]):
        return jsonify({"ok": False, "erro": "So um admin do servidor pode excluir canais."}), 403
    conexao = obter_bd()
    qtd = conexao.execute("SELECT COUNT(*) AS n FROM zap_canais WHERE grupo_id = ?", (grupo_id,)).fetchone()["n"]
    if qtd <= 1:
        conexao.close()
        return jsonify({"ok": False, "erro": "O servidor precisa ter pelo menos um canal."})
    conexao.execute("DELETE FROM zap_canais WHERE id = ? AND grupo_id = ?", (canal_id, grupo_id))
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})


@app.route("/zap/grupo/<int:grupo_id>/canal/<int:canal_id>/mensagens")
def zap_grupo_canal_mensagens(grupo_id, canal_id):
    if not session.get("usuario"):
        return jsonify({"mensagens": []}), 401
    usuario = session["usuario"]
    if not _membro_do_grupo(grupo_id, usuario):
    ...
    conexao = obter_bd()
    linhas = conexao.execute(
        "SELECT * FROM zap_grupo_mensagens WHERE grupo_id = ? AND canal_id = ? ORDER BY id ASC LIMIT 300", (grupo_id, canal_id)
    ).fetchall()
    conexao.close()
...
@app.route("/zap/grupo/<int:grupo_id>/canal/<int:canal_id>/enviar", methods=["POST"])
def zap_grupo_canal_enviar(grupo_id, canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    if not _membro_do_grupo(grupo_id, usuario):
    ...
    conexao = obter_bd()
    conexao.execute(
        "INSERT INTO zap_grupo_mensagens (grupo_id, canal_id, remetente, tipo, conteudo, criado_em) VALUES (?, ?, ?, 'texto', ?, ?)",
        (grupo_id, canal_id, usuario, texto, datetime.now().isoformat()),
    )
    conexao.execute("UPDATE zap_grupos SET passo_mensagem = 1 WHERE id = ?", (grupo_id,))
    _checar_ativacao_grupo(conexao, grupo_id)
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})
...
# ---------- Canais de voz (WebRTC em malha - varias pessoas no mesmo canal) ----------

def _presenca_canal(conexao, canal_id, usuario):
    return conexao.execute("SELECT * FROM zap_canal_presenca WHERE canal_id = ? AND usuario = ? COLLATE NOCASE", (canal_id, usuario)).fetchone()


@app.route("/zap/canal/<int:canal_id>/entrar", methods=["POST"])
def zap_canal_entrar(canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    canal = _canal_do_grupo(canal_id)
    if not canal or canal["tipo"] != "voz" or not _membro_do_grupo(canal["grupo_id"], usuario):
        return jsonify({"ok": False, "erro": "Canal de voz invalido."}), 403
    conexao = obter_bd()
    silenciado = conexao.execute("SELECT silenciado FROM zap_grupo_membros WHERE grupo_id = ? AND usuario = ? COLLATE NOCASE", (canal["grupo_id"], usuario)).fetchone()
    mudo_inicial = 1 if (silenciado and silenciado["silenciado"]) else 0
    conexao.execute(
        "INSERT INTO zap_canal_presenca (canal_id, usuario, entrou_em, camera, tela, mudo) VALUES (?, ?, ?, 0, 0, ?) "
        "ON CONFLICT(canal_id, usuario) DO UPDATE SET entrou_em = excluded.entrou_em",
        (canal_id, usuario, datetime.now().isoformat(), mudo_inicial),
    )
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True, "forcado_mudo": bool(mudo_inicial)})


@app.route("/zap/canal/<int:canal_id>/sair", methods=["POST"])
def zap_canal_sair(canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    conexao = obter_bd()
    conexao.execute("DELETE FROM zap_canal_presenca WHERE canal_id = ? AND usuario = ? COLLATE NOCASE", (canal_id, usuario))
    conexao.execute("DELETE FROM zap_canal_sinal WHERE canal_id = ? AND (de_usuario = ? COLLATE NOCASE OR para_usuario = ? COLLATE NOCASE)", (canal_id, usuario, usuario))
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})


@app.route("/zap/canal/<int:canal_id>/participantes")
def zap_canal_participantes(canal_id):
    if not session.get("usuario"):
        return jsonify([]), 401
    conexao = obter_bd()
    linhas = conexao.execute(
        """SELECT p.usuario, p.camera, p.tela, p.mudo, u.foto_perfil, m.silenciado FROM zap_canal_presenca p
           LEFT JOIN usuarios u ON u.usuario = p.usuario COLLATE NOCASE
           LEFT JOIN zap_canais c ON c.id = p.canal_id
           LEFT JOIN zap_grupo_membros m ON m.grupo_id = c.grupo_id AND m.usuario = p.usuario COLLATE NOCASE
           WHERE p.canal_id = ? ORDER BY p.entrou_em ASC""",
        (canal_id,),
    ).fetchall()
    conexao.close()
    avatar_de = lambda l: l["foto_perfil"] if l["foto_perfil"] else AVATAR_PADRAO + l["usuario"]
    return jsonify([
        {"usuario": l["usuario"], "avatar": avatar_de(l), "camera": bool(l["camera"]), "tela": bool(l["tela"]),
         "mudo": bool(l["mudo"]), "silenciado_pelo_admin": bool(l["silenciado"])}
        for l in linhas
    ])


@app.route("/zap/canal/<int:canal_id>/estado", methods=["POST"])
def zap_canal_estado(canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    dados = request.get_json() or {}
    conexao = obter_bd()
    presenca = _presenca_canal(conexao, canal_id, usuario)
    if not presenca:
        conexao.close()
        return jsonify({"ok": False, "erro": "Voce nao esta neste canal."})
    canal = _canal_do_grupo(canal_id)
    silenciado = conexao.execute("SELECT silenciado FROM zap_grupo_membros WHERE grupo_id = ? AND usuario = ? COLLATE NOCASE", (canal["grupo_id"], usuario)).fetchone()
    mudo_desejado = dados.get("mudo")
    if mudo_desejado is False and silenciado and silenciado["silenciado"]:
        conexao.close()
        return jsonify({"ok": False, "erro": "Um admin te silenciou neste servidor."})
    campos, valores = [], []
    if "mudo" in dados: campos.append("mudo = ?"); valores.append(1 if mudo_desejado else 0)
    if "camera" in dados: campos.append("camera = ?"); valores.append(1 if dados.get("camera") else 0)
    if "tela" in dados: campos.append("tela = ?"); valores.append(1 if dados.get("tela") else 0)
    if campos:
        valores += [canal_id, usuario]
        conexao.execute(f"UPDATE zap_canal_presenca SET {', '.join(campos)} WHERE canal_id = ? AND usuario = ? COLLATE NOCASE", valores)
        conexao.commit()
    conexao.close()
    return jsonify({"ok": True})


@app.route("/zap/canal/<int:canal_id>/sinal", methods=["POST"])
def zap_canal_sinal(canal_id):
    if not session.get("usuario"):
        return jsonify({"ok": False}), 401
    usuario = session["usuario"]
    dados = request.get_json() or {}
    ...
    conexao = obter_bd()
    conexao.execute(
        "INSERT INTO zap_canal_sinal (canal_id, de_usuario, para_usuario, tipo, dados, criado_em) VALUES (?, ?, ?, ?, ?, ?)",
...
@app.route("/zap/canal/<int:canal_id>/sinais")
def zap_canal_sinais(canal_id):
    if not session.get("usuario"):
        return jsonify({"sinais": []}), 401
    usuario = session["usuario"]
    desde = request.args.get("desde", 0, type=int)
    conexao = obter_bd()
    linhas = conexao.execute(
        "SELECT * FROM zap_canal_sinal WHERE canal_id = ? AND para_usuario = ? COLLATE NOCASE AND id > ? ORDER BY id ASC",
        (canal_id, usuario, desde),
    ).fetchall()
    conexao.close()
    return jsonify({"sinais": [{"id": l["id"], "de": l["de_usuario"], "tipo": l["tipo"], "dados": json.loads(l["dados"])} for l in linhas]})


def _canal_geral_do_grupo(grupo_id):
    conexao = obter_bd()
    linha = conexao.execute("SELECT id FROM zap_canais WHERE grupo_id = ? AND tipo = 'texto' ORDER BY id ASC LIMIT 1", (grupo_id,)).fetchone()
    conexao.close()
...
def zap_grupo_mensagens(grupo_id):
    """Mantido por compatibilidade - encaminha pro canal de texto padrao (#geral)."""
    canal_id = _canal_geral_do_grupo(grupo_id)
    if not canal_id:
        return jsonify({"mensagens": []})
    return zap_grupo_canal_mensagens(grupo_id, canal_id)
...
def zap_grupo_enviar(grupo_id):
    ...
    canal_id = _canal_geral_do_grupo(grupo_id)
    if not canal_id:
        return jsonify({"ok": False, "erro": "Este servidor nao tem canal de texto."})
    return zap_grupo_canal_enviar(grupo_id, canal_id)
