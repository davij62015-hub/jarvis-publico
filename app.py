"""
Jarvis - Site Publico
Chat + geracao/busca de imagem + conversor + rede social (JarvisWEB) com perfil, avatar e video por link.
...
def iniciar_bd():
    ...
    conexao.execute("""
        CREATE TABLE IF NOT EXISTS curtidas (
            post_id INTEGER NOT NULL, usuario TEXT NOT NULL, PRIMARY KEY (post_id, usuario)
        )
    """)
    conexao.execute("""
    ...
    conexao.execute("""
        CREATE TABLE IF NOT EXISTS seguidores (
            seguidor TEXT NOT NULL, seguido TEXT NOT NULL, PRIMARY KEY (seguidor, seguido)
        )
    """)
    conexao.commit()
    conexao.close()
...
def eh_video(url):
    if not url:
        return False
    url_baixo = url.lower()
    return any(ext in url_baixo for ext in [".mp4", ".webm", ".mov", "cdn.discordapp.com"]) and any(
        ext in url_baixo for ext in [".mp4", ".webm", ".mov"]
    )
...
.caixa { background:#0d0d0d; padding:40px; border-radius:16px; border:1px solid #ffffff33; text-align:center; width:320px; max-width:90vw; box-shadow:0 0 30px #ffffff11; }
.logo-img { width:70px; height:70px; border-radius:50%; margin-bottom:10px; box-shadow:0 0 16px #ffffff55; }
h2 { margin:10px 0 20px; }
input { width:100%; padding:12px; margin-top:10px; border-radius:8px; border:1px solid #ffffff33; background:#000000; color:#f2f2f2; }
button { width:100%; padding:12px; margin-top:16px; border-radius:8px; border:none; background:#ffffff; color:#000000; font-weight:bold; cursor:pointer; }
.trocar { margin-top:16px; font-size:13px; color:#aaaaaa; cursor:pointer; text-decoration:underline; }
...
.link-rede { background:#1a1a1a; color:#f2f2f2; border:1px solid #ffffff33; border-radius:8px; padding:10px; text-align:left; cursor:pointer; margin-bottom:16px; display:block; text-decoration:none; }
...
.botao-mic { background:#1a1a1a !important; color:#f2f2f2 !important; border:1px solid #ffffff33 !important; padding:12px 14px !important; display:flex; align-items:center; }
...
async function buscarImagem() {
    ...
    adicionarMensagem("jarvis", "Encontrei essa (imagem real, licenca aberta):<br><img src='" + dados.url + "'>");
}
...
<body>
<div class="topo"><a href="/painel" class="voltar">&#8592;</a><span class="titulo-topo">JarvisWEB</span></div>
<div class="container">
  {painel_admin}
...
.voltar { color:#ffffff; text-decoration:none; font-size:20px; }
.container { max-width:600px; margin:0 auto; padding:16px; }
.cabecalho-perfil { display:flex; align-items:center; gap:20px; margin-bottom:20px; }
...
<div class="container">
  <div class="cabecalho-perfil">
    ...
    <div>
      ...
      <div class="stats">
        <div><b>{qtd_posts}</b>posts</div>
        <div><b>{qtd_seguidores}</b>seguidores</div>
        <div><b>{qtd_seguindo}</b>seguindo</div>
      </div>
      {botao_seguir}
    </div>
  </div>
...
def login():
    if request.method == "POST":
        ...
        linha = conexao.execute("SELECT * FROM usuarios WHERE usuario = ? COLLATE NOCASE", (usuario,)).fetchone()
        conexao.close()
...
def painel():
    ...
    pagina = PAGINA.replace("{usuario}", usuario).replace("{selo_dev}", selo).replace("{avatar_url}", avatar)
    return pagina
...
def chat():
    ...
    linhas = conexao.execute(
        "SELECT remetente, texto FROM mensagens WHERE usuario = ? ORDER BY id DESC LIMIT 12", (usuario,)
    ).fetchall()
    historico_mensagens = [
...
def buscar_imagem():
    ...
    if not termo or not requests:
        return jsonify({"url": ""})
    try:
        resposta = requests.get("https://api.openverse.org/v1/images/", params={"q": termo, "page_size": 1}, timeout=8)
        dados = resposta.json()
        resultados = dados.get("results", [])
        if not resultados:
            return jsonify({"url": ""})
        return jsonify({"url": resultados[0].get("url", ""), "fonte": resultados[0].get("source", "")})
    except Exception:
        return jsonify({"url": ""})
...
def perfil(nome_usuario):
    ...
    for p in posts:
        if p["imagem"]:
            ...
            texto_curto = (p["texto"] or "")[:40]
            itens_grade += f'<div class="grade-item sem-midia">{texto_curto}</div>'
    ...
    pagina = PAGINA_PERFIL.replace("{nome_usuario}", nome_real)
    pagina = pagina.replace("{avatar_url}", avatar)
    pagina = pagina.replace("{selo}", selo)
    pagina = pagina.replace("{qtd_posts}", str(len(posts)))
    pagina = pagina.replace("{qtd_seguidores}", str(qtd_seguidores))
    pagina = pagina.replace("{qtd_seguindo}", str(qtd_seguindo))
    pagina = pagina.replace("{botao_seguir}", botao_seguir)
    pagina = pagina.replace("{editor_perfil}", editor_perfil)
    pagina = pagina.replace("{itens_grade}", itens_grade)
    return pagina
...
def rede_postar():
    ...
    conexao = obter_bd()
    conexao.execute(
        "INSERT INTO posts (usuario, texto, imagem, video, criado_em) VALUES (?, ?, ?, ?, ?)",
        (usuario, texto, caminho_imagem, video or None, datetime.now().isoformat()),
    )
    conexao.commit()
    conexao.close()
    return jsonify({"ok": True})
...
def rede_feed():
    ...
    for p in posts:
        ...
        resultado.append({
            "id": p["id"], "usuario": p["usuario"], "texto": p["texto"],
            "imagem": p["imagem"], "video": p["video"], "avatar": avatar,
            "curtidas": curtidas, "curtido": curtido, "seguindo": seguindo, "verificado": verificado,
